...
for SCENE_PATH in "$ROOT"/*; do
    if [ -d "$SCENE_PATH" ]; then
        ...
        for i in {1..4}; do
            ...
            # CSV 개별 회차 기록
            if [ ! -f "$CSV_FILE" ]; then
                echo "scene,run,SSIM,PSNR,LPIPS,TrainTime(s),RenderTime(s),MaxVRAM(MB)" > "$CSV_FILE"
            fi
            echo "$SCENE,run${i},$SSIM,$PSNR,$LPIPS,$TRAIN_TIME,$RENDER_TIME,$VRAM_MAX" >> "$CSV_FILE"
        ...
        python ../update_sheet.py "$SHEET_NAME" "$SCENE" "$AVG_SSIM" "$AVG_PSNR" "$AVG_LPIPS" "$AVG_TRAIN_TIME" "$AVG_RENDER_TIME" "$AVG_VRAM"
