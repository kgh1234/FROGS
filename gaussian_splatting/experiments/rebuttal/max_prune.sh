...
OUTPUT_ROOT="../../rebuttal/max_pruning/$SCENE_NAME"
CSV_FILE="$OUTPUT_ROOT/metrics_summary_$SCENE_NAME.csv"
SHEET_NAME="rebuttal_maxpruning"

# 여러 alpha 값 반복
PRUNING_MAX=(0.0 0.25 0.5 0.75 1.0)
...
for SCENE_PATH in "$ROOT"/*; do
    if [ -d "$SCENE_PATH" ]; then
        ...
        for pruning_max in "${PRUNING_MAX[@]}"; do
            OUT_DIR="$OUTPUT_ROOT/${SCENE}iter${K}"
            if [ -d "$OUT_DIR" ]; then
            ...
            echo "====================================="
            echo "Processing scene: $SCENE | ITER=$pruning_max"
            echo "====================================="
            ...
            python train_all.py -s "$SCENE_PATH" -m "$OUT_DIR" --mask_dir "$MASK_DIR" --prune_iterations 600 900 1200 --pruning_max $pruning_max --eval
            TRAIN_END=$(date +%s)
            ...
            python ../../update_sheet.py "$SHEET_NAME" "${SCENE}_${K}" "$SSIM" "$PSNR" "$LPIPS" "$MIOU" "$TRAIN_TIME" "$RENDER_TIME" "$FPS" "$VRAM_MAX" "$GAUSSIAN_COUNT"
