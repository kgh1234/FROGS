...
OUTPUT_ROOT="../../rebuttal/covscore/$SCENE_NAME"
CSV_FILE="$OUTPUT_ROOT/metrics_summary_$SCENE_NAME.csv"
SHEET_NAME="rebuttal_hyperparameter"

# 여러 alpha 값 반복
COVSCORES=(0.5 0.6 0.7 0.8 0.9 1.0)
...
for SCENE_PATH in "$ROOT"/*; do
    if [ -d "$SCENE_PATH" ]; then
        ...
        for COVSCORE in "${COVSCORES[@]}"; do
            OUT_DIR="$OUTPUT_ROOT/${SCENE}iter${COVSCORE}"
            if [ -d "$OUT_DIR" ]; then
            ...
            echo "====================================="
            echo "Processing scene: $SCENE | ITER=$COVSCORE"
            echo "====================================="

            echo " Training 시작..."
            TRAIN_START=$(date +%s)
            LOGFILE="vram_${SCENE}_${COVSCORE}.log"
            nvidia-smi --query-gpu=memory.used --format=csv,nounits,noheader -l 2 > "$LOGFILE" &
            VRAM_PID=$!

            python train_all.py -s "$SCENE_PATH" -m "$OUT_DIR" --mask_dir "$MASK_DIR" --prune_iterations 600 900 1200 --cov_threshold $COVSCORE --eval
            ...
            python ../../update_sheet.py "$SHEET_NAME" "${SCENE}_${COVSCORE}" "$SSIM" "$PSNR" "$LPIPS" "$MIOU" "$TRAIN_TIME" "$RENDER_TIME" "$FPS" "$VRAM_MAX" "$GAUSSIAN_COUNT"
