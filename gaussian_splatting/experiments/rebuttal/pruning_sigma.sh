...
OUTPUT_ROOT="../../rebuttal/soft_k/$SCENE_NAME"
CSV_FILE="$OUTPUT_ROOT/metrics_summary_$SCENE_NAME.csv"
SHEET_NAME="rebuttal_soft_k"
...
for SCENE_PATH in "$ROOT"/*; do
    if [ -d "$SCENE_PATH" ]; then
        ...
        for K in "${KS[@]}"; do
            ...
            echo "====================================="
            echo "Processing scene: $SCENE | ITER=$K"
            echo "====================================="
            ...
            python train_all.py -s "$SCENE_PATH" -m "$OUT_DIR" --mask_dir "$MASK_DIR" --prune_iterations 600 900 1200 --threshold_prune_k $K --eval
            TRAIN_END=$(date +%s)
